...
def grabFITS(subhalo_id, snap, img_dir, bands =  ['wfc3_ir_f160w'], overwrite = False):
    #if len(sys.argv) < 4:
    ...
    tmp_dir = '/home/AstroPhysics-Shared/PROJECTS/MPhys_Villforth/DATA/IMG_dir/HDF5_dir/'

    # default image names
    #fitsname = '%s%02d%07d.fits' %(img_dir, snap, subhalo_id)
    
    fitsname = '%ssfid_{}_snap_{}_axes_{}_band_{}.fits' %(img_dir)
    fitsname = '%sSFID_%07d_SNAP_%02d.fits' %(img_dir, subhalo_id, snap)
    ...
    if os.path.isfile(fitsname) and overwrite == False:
        return print('%s already exists' %fitsname)
        sys.exit(0)
    ...
    table_dir = '/home/AstroPhysics-Shared/PROJECTS/MPhys_Villforth/DATA/DF_dir/'

    # get merger label info for our subhalo
    object_table_name = 'objects_master_22_10_28.parquet'
