...
'''
if len(sys.argv) < 3:
    print('Usage: ./hdf52fits.py subhaloID snap')
    sys.exit(0)
...
print('Read merger table from %s%s' %(table_dir, merger_table_name))
print('Time: ' + str(datetime.timedelta(seconds=(time.time() - start_time))))
sys.stdout.flush()

# index of the galaxy in the table
idx = np.where(np.logical_and(merger_table['subhalo_id'] == subhalo_id,
                              merger_table['subhalo_snap'] == snap))[0]
...
time_table = Table.read(table_dir + 'lookback_times.csv', format='ascii.csv')
header['HIERARCH redshift'] = float(time_table['redshift'][time_table['snapshot']
                                                           == snap])
header['HIERARCH lookback_time'] = (float(time_table['lookback_time_planck15'][time_table['snapshot']
                                                                               == snap]), 'from Planck 2015 cosmology')
...
# keys_to_copy = ['ratio_last', 'snap_last', 'dt_last',
...
for i in range(n_keys):
    key = keys_to_copy[i]
    dtype = types[i]
    header['HIERARCH ' + key] = (dtype(merger_table[key][idx]), comments[i])
