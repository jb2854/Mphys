...
def grabFITS(subhalo_id, snap, img_dir):
    ...
    img_dir = '/mnt/jb2854-XDrive/Physics/StudentProjects/MPhys/2022-23/IllustrisTNG-Mergers/DF_dir/'
    FITS_dir = img_dir + 'FITS_dir/'
    # temporary storage for HDF5 files
    tmp_dir = img_dir + 'HDF5_dir/'

    # default image names
    #fitsname = '%s%02d%07d.fits' %(img_dir, snap, subhalo_id)
    
    fitsname = '%ssfid_{}_snap_{}_axes_{}_band_{}.fits' %(FITS_dir)
    ...
    if os.path.isfile(fitsname.format(SFID, snap, axes, band)):
        print('%s already exists' %fitsname.format(SFID, snap, axes, band))
        sys.exit(0)
    ...
    object_table_name = 'objects_master_22_10_25.parquet'
    ...
    header['HIERARCH CDELT1'] = (header['stellar_half_mass_radius'] *
                                 img_params['size']/(Planck15.H(0).value/100) / n_pix,
                                 'kpc/pixel')
    print(header)
    ...
    for axes in ['0,1', '0,2', '1,2']:
        ...
        for band in bands:
            ...
            hdul.writeto(fitsname.format(SFID, snap, axes, band), overwrite = True)
